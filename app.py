...
with col_pwr:
    ...
    with cc3: power_reserve = st.selectbox("Запас (Мощность)", [0, 10, 20, 30], format_func=lambda x: f"{x}%", index=2)
...
# Расчет БП (по мощности и по количеству модулей на БП)
...
num_psu_by_power = math.ceil(power_with_reserve_kw / (psu_power / 1000))
num_psu_by_mod = math.ceil(total_modules / modules_per_psu)
num_psu = max(num_psu_by_power, num_psu_by_mod)
...
with st.expander("Блоки питания", expanded=True):
    st.markdown(f"""
    - **Схема коммутации**: {modules_per_psu} модулей на 1 БП ({psu_power}W)
    - **Пиковое потребление экрана**: {peak_power_screen_kw:.1f} кВт
    - **Расчетное количество по мощности (+{power_reserve}%)**: {num_psu_by_power} шт.
    - **Расчетное количество по хвостам (коммутации)**: {num_psu_by_mod} шт.
...
with st.expander("Вводная Сеть", expanded=True):
    ...
    - **Ток**: {current:.1f} А (с учетом запаса)
