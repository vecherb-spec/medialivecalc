...
# Проверка портов вынесена на всю ширину для акцента
...
if load_per_port > 90 and required_ports <= available_ports:
    st.warning("⚠️ Нагрузка >90%! Рекомендуется процессор с большим количеством портов для запаса прочности.")
...
with col_pwr:
    ...
    with cc2: power_reserve = st.selectbox("Запас мощности БП", [0, 10, 20, 30], format_func=lambda x: f"{x}%", index=2)
...
with col_zip:
    ...
    if reserve_enabled:
        with st.container():
            ...
            with zc2:
                st.markdown("<div style='margin-top: 28px;'></div>", unsafe_allow_html=True) # выравнивание
...
# ==========================================
# БЛОК 5: РЕЗУЛЬТАТЫ И ОТЧЕТ
# ==========================================
st.markdown('<div class="section-header">📊 Итоговые расчеты</div>', unsafe_allow_html=True)
...
st.subheader("📋 Спецификация комплектующих")

with st.expander("📦 Модули и Кабинеты", expanded=True):
    st.markdown(f"""
    - **Тип модулей:** Qiangli {tech} P{pixel_pitch} {env_key}
    - **Матрица модулей:** {modules_w} (Ш) × {modules_h} (В)
    - **Всего модулей (с учетом ЗИП {reserve_modules} шт):** **{total_modules_order} шт.**
    """)
    if "кабинетах" in mount_type:
        st.markdown(f"""
        - **Кабинеты:** {cabinet_model} ({cabinet_width}×{cabinet_height} мм)
        - **Матрица кабинетов:** {cabinets_w} (Ш) × {cabinets_h} (В)
        - **Итого кабинетов:** **{total_cabinets} шт.**
        """)

with st.expander("🔌 Электроника и Управление", expanded=True):
    st.markdown(f"""
    - **Процессор:** {processor}
    - **Приёмные карты ({receiving_card}):** {num_cards} шт + ЗИП = **{num_cards_reserve} шт.**
    - **Блоки питания ({psu_power}W):** {num_psu} шт + ЗИП = **{num_psu_reserve} шт.**
    """)

with st.expander("⚡ Кабели, Каркас и Сеть", expanded=False):
    st.markdown(f"""
    - **Подключение:** {power_phase}, Автомат {breaker}А, Кабель {cable_section}
    - **Общий вес экрана:** ~{total_weight:.1f} кг
    """)
    if "Монолитный" in mount_type:
        st.markdown(f"""
        - **Магниты ({magnet_size}):** {math.ceil(total_modules * 4 / 500) * 500} шт. (округлено до уп.)
        - **Профиль:** Вертикальных {vert_profiles} шт, Горизонтальных {horiz_profiles} шт.
        """)
