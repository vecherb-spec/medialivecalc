...
if "height_mm" not in st.session_state:
    st.session_state.height_mm = 2160  # 16:9 для 3840
...
with col1:
    ...
    st.session_state.height_mm = height_mm  # сохраняем ручное изменение

    screen_type = st.radio("Тип экрана", ["Indoor", "Outdoor"], index=0)

# ... (все остальные поля ввода остаются без изменений: монтаж, шаг, кабинет, частота, система, процессор, проверка портов и т.д.)
