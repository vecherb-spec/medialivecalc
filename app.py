...
# Кабинет для "В кабинетах"
cabinet_model = "QM Series (640×480 мм)"
cabinet_width = 640
cabinet_height = 480
...
if mount_type == "В кабинетах":
    ...
    cabinet_options = [
        ("QM Series (640×480 мм, indoor)", 640, 480, 20.0),
        ("MG Series (960×960 мм, outdoor/indoor)", 960, 960, 40.0),
        ("QF Series (500×500 мм, rental/indoor)", 500, 500, 13.5),
        ("QS Series (960×960 мм, outdoor)", 960, 960, 45.0),
        ("Custom", 640, 480, 20.0)
    ]
    selected = st.selectbox("Модель кабинета", [opt[0] for opt in cabinet_options], index=0)
    for name, w, h, wt in cabinet_options:
        if name == selected:
            cabinet_model = name
            cabinet_width = w
            cabinet_height = h
            cabinet_weight_per = wt
            break
    if selected == "Custom":
        cabinet_width = st.number_input("Ширина кабинета (мм)", value=640)
        cabinet_height = st.number_input("Высота кабинета (мм)", value=480)
        cabinet_weight_per = st.number_input("Вес одного кабинета (кг)", value=20.0)
...
if st.button("Рассчитать", type="primary", use_container_width=True):
    ...
    # Расчёт сети (ток, кабель, автомат) — ДО вывода!
    ...
    weight_carcas = total_profile_length * 2  # пример 2 кг/м
...
<span style="color:#00ff9d;">███</span> — модуль установлен
    ...
    # Вывод отчёта (все переменные уже рассчитаны)
